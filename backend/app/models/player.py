...
class Player(Base):
    ...
    profiles = relationship(
        "DevelopmentalProfile", back_populates="player", cascade="all, delete-orphan"
    )
    learning_sessions = relationship(
        "LearningSession", back_populates="player", cascade="all, delete-orphan"
    )
    task_attempts = relationship("TaskAttempt", back_populates="player")
    reinforcement_config = relationship(
        "ReinforcementConfig",
