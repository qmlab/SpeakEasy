...
def find_object(request: FindObjectRequest, db: Session = Depends(get_db)):
    ...
    image = (
        db.query(ObjectImage).filter(ObjectImage.id == request.object_image_id).first()
    )
    if not image:
    ...
    bounding_boxes = (
        ...
        .filter(BoundingBox.object_image_id == request.object_image_id)
        .all()
    )

    if not bounding_boxes:
        raise HTTPException(
            status_code=400, detail="This image has no bounding boxes defined"
        )
    ...
    for box in bounding_boxes:
        hit, score = ScoringService.check_tap_location(
            request.tap_x, request.tap_y, box.x, box.y, box.width, box.height
        )
...
def get_random_image_with_boxes(
    ...
    if not images:
        raise HTTPException(
            status_code=404, detail="No images with bounding boxes found"
        )
