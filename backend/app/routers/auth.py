...
def apple_sign_in(request: AppleSignInRequest, db: Session = Depends(get_db)):
    existing_player = (
        db.query(Player).filter(Player.apple_user_id == request.apple_user_id).first()
    )
    ...
    new_player = Player(
        name=name, apple_user_id=request.apple_user_id, email=request.email
    )
    db.add(new_player)
    db.commit()
...
def get_player_by_apple_id(apple_user_id: str, db: Session = Depends(get_db)):
    player = db.query(Player).filter(Player.apple_user_id == apple_user_id).first()
...
def guest_sign_in(request: GuestSignInRequest, db: Session = Depends(get_db)):
    existing_player = (
        db.query(Player).filter(Player.device_id == request.device_id).first()
    )
    ...
    short_id = (
        request.device_id[-6:].upper()
        if len(request.device_id) >= 6
        else request.device_id.upper()
    )
    name = f"Guest_{short_id}"
    new_player = Player(name=name, device_id=request.device_id, is_guest="true")
    db.add(new_player)
    db.commit()
    db.refresh(new_player)
...
def get_player_by_device_id(device_id: str, db: Session = Depends(get_db)):
    player = db.query(Player).filter(Player.device_id == device_id).first()
